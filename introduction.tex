...
In this work, we propose a machine learning system that learns to predict for an ATP
...
the system generalizes the knowledge about favourable precedences across problems with different signatures.

Our recommender uses a graph convolutional neural network \cite{kipf2017semisupervised}
to represent the problem structure. It learns from the ATP performance on selected problems
and \emph{pairs of randomly sampled} precedences. This information is then used to train
a \emph{symbol cost model}, which realizes the recommendation by simply sorting 
the problem's symbols according the obtained costs. 
...
and is, to the best of our knowledge, a first method able to propose good symbol precedences automatically 
using a non-linear transformation of the input problem structure.


The rest of this paper is organized as follows. \ldots
% explicit contributions (mixed with ``as follows'')?

\todo{Asi bych znovu (po abstraktu) nerikal, kolik nam vyjde. Spis jen ze sekce ta a ta je to tom, ze se tam evaluu s vampirem na TPTP. }
When coupled with the ATP Vampire and trained on problems from the TPTP library,
the recommender improves on the state-of-the art general-purpose precedence generating scheme
\texttt{invfreq} by more than \SI{4}{\percent}.
