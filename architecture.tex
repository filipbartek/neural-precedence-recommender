...
Let $(P, \PrecBetter, \PrecWorse)$ be a training example ($\Better{\PrecBetter}{\PrecWorse}$).
The precedence cost classifies this example correctly if $C(\PrecBetter) < C(\PrecWorse)$,
or alternatively $C_2(\PrecBetter, \PrecWorse) = C(\PrecBetter) - C(\PrecWorse) < 0$.
...
\begin{align*}
L(P, \PrecBetter, \PrecWorse)
&= - \log \sigmoid C_2(\PrecBetter, \PrecWorse)
...
Note that the classifier cannot simply train $C_2$ to output a negative number on all pairs of precedences,
because $C_2$ is a difference of two precedence costs.
