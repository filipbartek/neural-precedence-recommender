...
that, when presented to an \gls{atp} along with the problem $P$,
yields a proof in few iterations of the saturation loop.
...
that are likely to yield a proof in as few iterations of the saturation loop as possible.
...
\item \Cref{sec:output}: The output function extracts symbol costs from the symbol embeddings.
...
we define a precedence cost function $C$ on top of $c$
in a way that ensures that minimizing $C$ corresponds to sorting the symbols by $c$,
so that finding the precedence that minimizes $C$ can be done efficiently and precisely.
Then we proceed to train $C$ on the proxy task of ranking the precedences.
...
Approaching the task as binary classification of precedences offers some promising options,
such as learning to detect the precedences that yield a successful proof search within the allocated time.
However, we cast the task as an instance of score-based ranking problem \cite{Mohri2018,Burges2005}
% Mohri2018: p. 239, chapter 10 Ranking
by training a classifier to decide which of a pair of precedences is better based on their costs.
