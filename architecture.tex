...
\item \Cref{sec:ranking}: Ranking converts the symbol costs into a symbol precedence.
...
The symbol cost of each symbol is computed by passing the symbol's embedding through a fully-connected layer
with a single output unit.
No activation function is applied on the output values.
\todo{FB: Explain why.}

It is possible to use a more complex output layer,
...
Let $\theta$ denote the vector of all the parameters of the \gls{gcn} and the output layer.
Given an input problem $P$ with signature $\Sigma = (s_1, \ldots, s_n)$,
we denote the cost of symbol $s_i$ as $c_{\theta,P}(i)$,
or simply $c(i)$ in case the parameters $\theta$ and the problem $P$ are fixed in the context.
