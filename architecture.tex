...
\subsection{System overview} \todo{MS: tuhle subsekci by slo zrusit a usetriti misto. Rozcestnik je beztak neco ``meta''.}
...
\todo{MS: Ja vim, ze uz je to nuda psat ty dlouhy a presny vety,
ale ``timely fashion'' uz je mozna moc neformalni a
a ``precedence does not solve a problem''.}
...
\todo{MS: tady mi to ted (po neuralnim prelim) uz prijde asi dobry!}
...
\todo{MS: o dost lespi verze v podstate te samy vety z konce prvniho odstavnce "System overview"}
...
\todo{MS: je to schalne, ze tam mas vsude ``converts''? V nekterych bodech by asi byla k dispozici prilehavejsi slova.
Treba sorting converts uz zni trochu sroubovane. }
\end{itemize}
}{
sections (see also \cref{fig:architecture}).
}
...
\item \todo[inline]{MS: (Almost)} Perfect sub-expression sharing:
...
\todo{FB: Is this clear at this point? MS: jj, ted uz jo!}
...
because the problem $P$ and the parameters $\theta$ are fixed in each respective context.\todo{MS: I like.}
...
\subsubsection{Precedence cost} \todo{MS: subsubsections jsou trochu e-e}
...
= \mathbb{E}_i [c(i)]
\end{multline*} \todo{MS: jeste jsem zapomnel napsat, ze tenhle vzorecek je hezky!}
...
\subsubsection{Cost minimization by sorting} \todo{MS: subsubsections jsou trochu e-e}
\todo{MS: tady spis chybi rict, proc to lema vybev potrebujem.}
\begin{lemma}
...
\todo{MS: libi se mi, ze tu je tenhle vysvetlujici odstavec!}
Instead of learning to classify one or more precedences on each problem as the best,
\todo{MS: To by asi stejne nepomohlo. Protoze jak pak ty dobre hledat pro novy problem?}
\todo{MS: ale dalo by se znimit, ze v PAAR paperu jsme na to zkusili jit \emph{regresi} (as opposed to classification),
tj predikovat primo nejakou real-valued kvalitu kazde konkretni precedence.}
...
we are pushing $C(\PrecBetter)$ down and $C(\PrecWorse)$ up.\todo{MS: super, tohle mi takhle nejak nahlas nikdy nedoslo :)}

The loss function is clearly differentiable with respect to the symbol costs,
\todo{Shall we include the derivative? MS: Spis ne.}
