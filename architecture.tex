...
\todo{FB: Too vague? MS: v tomhle momente mi to prijde dobry!} 

The recommender consists of modules that perform specific subtasks (see \cref{fig:architecture}):
\begin{enumerate}
\item Graphifier\todo{MS: Zatim nevidim Graphifier ve \cref{fig:architecture}.} converts the input \gls{cnf} problem into a problem graph.
...
\todo{MS: Footnote asi remove, ale neexistuje citace pro ``heterogeneous directed graph''?}
Each of the nodes is labeled with a node type,
and each edge is labeled with an edge type,
informing\todo{MS: maybe better ``defining''?} the heterogeneous nature of the graph.
...
\item Term-sharing is employed:\todo{MS: Mozna presneji ``perfect sub-expression sharing''? 
Predpokladam ze dva ruzne ground literaly p(c) tam budou taky jen jendnou?}
...
The symbol embeddings are modeled\todo{MS: embeddings are ``output''? } by a \gls{rgcn} \cite{Schlichtkrull2017},
\todo{Can I use the term \gls{rgcn} when I use a modified version? Namely, the activation is called earlier. MS: sounds good.}
...
\todo{Ensure terms are clear (preferably defined): embedding, vector, trainable vector, feature vector.}
...
\todo{MS: ``message'' occurs for the first time. 
Maybe ``a message'', or explain beforehand.}
...
$$
h_d^{(l+1)} =
\sum_{r \in \mathcal{R}} \sigma \Parentheses{\sum_{j \in \mathcal{N}_s^r} \frac{1}{c_{s,d}} h_s^{(l)} W_r^{(l)}}
$$
%$$
%h_i^{(l+1)} =
