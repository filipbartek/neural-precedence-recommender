...
\item Output function \cite{Zhou2018} converts the symbol embeddings into a vector of symbol costs.
...
FB: Staci predestrit, ze detaily vysvetlim pozdeji?}
...
In the simple case that the module is a dense layer with weights $W_r^{(l)}$,
\todo{MS: hnidopich nevi, co to znamena ``dense layer''}
the message is $h_s^{(l)} W_r^{(l)}$.
...
Our experiments showed no significant improvement when a hidden layer is added,
arguably because the underlying \gls{gcn} learns a sufficiently complex transformation.
...
&< 0
\end{align*}
The final inequality is due to the assumption that $k-l < 0$ and
$c(\pi(l)) - c(\pi(k)) > 0$.
...
Note that the precedence that minimizes $C$
orders the symbols with the lowest cost as the last,
prioritizing them for early inferences.
...
from executions of an \gls{atp} on a set of problems with various precedences.
...
such that $\PrecBetter$ solves $P$ in fewer iterations of the saturation loop than $\PrecWorse$,
...
which can alternatively be expressed as $s(\PrecBetter, \PrecWorse) = C(\PrecBetter) - C(\PrecWorse) < 0$.
We consider the value $s(\PrecBetter, \PrecWorse)$
to be the score of the (ordered) precedence pair $(\PrecBetter, \PrecWorse)$.
...
A common approach to training a binary classifier is to relax the non-differentiable accuracy $[s(\PrecBetter, \PrecWorse) < 0]$
